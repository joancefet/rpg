    ...
    if(mysql_num_rows($sql) == 1){
      ...
      elseif(($select['pagina'] == "attack") OR ($select['pagina'] == "attack-trainer") OR ($select['pagina'] == "duel"))
        ...
        function status_check(){
          $.get("attack/duel/status_check.php?duel_id="+<? echo $duel_id; ?>+"&sid="+Math.random(), function(data) {
            if(data == 0){
...
<form method="post">
  <center>
    ...
    <table width="300">
      ...
      <tr>
        <td><img src="images/icons/silver.png" title="Silver" style="margin-bottom:-3px;" /> <?php echo $txt['money']; ?></td>
        <td><input type="text" name="bedrag" value="<?php if(!empty($_POST['bedrag'])) echo $_POST['bedrag']; else echo 0; ?>" class="text_long"></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
        <td><button type="submit" name="duel" class="button" <?php echo $button; ?>><?php echo $txt['button_duel']; ?></button></td>
      </tr>
  </table>
  </center>
</form>